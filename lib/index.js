...
const getAccountBalancesUri = (baseUrl, sessionToken, accountType) => {
  return baseUrl + '/cbs/accountBalances?sessionToken='+sessionToken+'&accountType='+accountType
}
...
const getAccountBalancesPure = async (sessionToken, accountType, url) => {
  const transferResult = (
    await fetchJson(
      getAccountBalancesUri(url, sessionToken, accountType),
